...
public:
    ...
    void Operate(std::shared_ptr<OpNode> node) override {
        ...
        node->SetAttribute("plugin.bge.piece.sprite-sheet-animator.command", "sprite-sheet create|rename | animation-sequence create|rename | animation-profile create|mode|rename | actor animation bind|auto");
        node->SetAttribute("plugin.bge.piece.sprite-sheet-animator.flags", "--id --to --image --columns --rows --frame-size --alpha-source --alpha-cutoff --preserve-alpha --no-preserve-alpha --transparent-background --opaque-background --frames --fps --loop --tier-counter --map --profile --default --mode --sequence --motion --terrain --mood --capability --show --color");
        ...
        log << "  command=sprite-sheet create|rename | animation-sequence create|rename | animation-profile create|mode|rename | actor animation bind|auto\n";
    }
};
